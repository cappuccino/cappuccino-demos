...
function _loadLevelData()
{
    ...
    var rand_no = Math.floor(5 * Math.random()),
        levelData = [_levelArray objectAtIndex:rand_no];
        ...
        while (b < 5)
        {
            while (a < 5)
            {
                var bounds = CGRectMake(0, 0, kTileWidth, kTileHeight),
                    location = CGPointMake(32 + 2 + a * kTileWidth, 28 + (76 + b * 7) + b * kTileHeight),

                    tile = [[LOTile alloc] init];
...
- (CPArray)tilesAdjacentTo:(LOTile)tile
{
  if (!tile)
    ...
    var tileFrame = [tile frame],
        centerOfTile = CGPointMake(CGRectGetMidX(tileFrame), CGRectGetMidY(tileFrame)),
    ...
    var i,
        results = [CPMutableArray arrayWithObject:tile];

    for (i = 0; i < 4; i++)
    {
        var neighbor = neighbors[i],

            neighboringTile = [_rootLayer hitTest:CGPointMake(centerOfTile.x + neighbor.x, centerOfTile.y + neighbor.y - 26)];
